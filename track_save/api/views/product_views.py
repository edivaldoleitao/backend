...
from track_save.api.controllers.price_controller import (
    search_products_with_price_by_name,
)
...
@csrf_exempt
@require_GET
def search_products_with_price_view(request):
    query = request.GET.get("q")

    if not query:
        return HttpResponseBadRequest("Parâmetro de busca 'q' é obrigatório.")

    try:
        products = search_products_with_price_by_name(query)

        if not products:
            return HttpResponseNotFound("Nenhum produto encontrado com esse termo.")
